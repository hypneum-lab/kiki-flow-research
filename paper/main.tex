...
\paragraph{Asymmetric drift not integrated.} Eq.~\eqref{eq:drift-asym}
defines $b^{\mathrm{asym}} = J^{\mathrm{asym}}\rho$ as a linear
non-conservative drift, but the current solver does not apply a
second splitting pass for it: the JKO step sees only the
$J^{\mathrm{sym}}$ component of Eq.~\eqref{eq:freeenergy}, and the
multiscale loop integrates $b^{\mathrm{phono}}$ but not
$b^{\mathrm{asym}}$. The coupling-modes ablation reflects this:
\textsc{symmetric-*} and \textsc{*-full} modes coincide to six
decimals. Adding a linear-drift splitting step in the style of
\cite{gallouet2017splitting} is a natural extension --- it would
re-activate the effect of the $31.4\%$ of $\|J\|_F$ carried by
$J^{\mathrm{asym}}$ and could shift the transient contraction rate
$\hat\lambda$ without necessarily shifting the stationary entropy.
We defer this to future work because the correct discretization
(mass-preserving forward Euler vs.\ semi-Lagrangian advection vs.\
matrix-exponential integrator) is not uniquely determined by the
formalism alone and merits its own numerical study.
